    ...
    def set_up_interactions(self,VA,VB,VC,VAB,VAC,VBC,kac0,kbc0,Aab,Aac,Abc):
        ...
        class CustomPlugin(KMCRateCalculatorPlugin):
            def initialize(self):
                ...
                #Va = np.array([VA(x[0]) for x in self.outer_class.get_space_point()])
                #Vb = np.array([VB(x[0]) for x in self.outer_class.get_space_point()])
                #Vc = np.array([VC(x[0]) for x in self.outer_class.get_space_point()])
                #Vs = np.zeros(outer_class.size,dtype=float)
                #Vab = np.array([VAB(x[0]) for x in self.outer_class.get_space_point()])
                #Vac = np.array([VAC(x[0]) for x in self.outer_class.get_space_point()])
                #Vbc = np.array([VBC(x[0]) for x in self.outer_class.get_space_point()])
                #self.Vs = {'A':Va,'B':Vb,'C':Vc,'S':Vs,
                #    'AB':Vab,'BA':Vab,'AC':Vac,'CA':Vac,'BC':Vbc,'CB':Vbc}
            def rate(self,coords,types_before,types_after,rate_constant,process_number,global_coordinate):
                ...
                if types_before.__len__()==1:
                    ...
                    #print([coords[i] for i in range(3)])
                    ...
                    #print(rate_constant*np.exp((-E_after+E_before)/2))
                    return rate_constant*np.exp((-E_after+E_before)/2)
                
                #if types_before.__len__()==1:
                #    return rate_constant* np.exp(-(self.Vs[types_after[0]+types_before[0]][self.outer_class.I(global_coordinate+coords[0])]-
                #                                   self.Vs[types_before[0]][self.outer_class.I(global_coordinate+coords[0])]))
                ## diffusion move
                #else:
                #    #print([self.Vs[types_before[i]] for i in range(types_before.__len__())])
                #    #print([self.outer_class.I(global_coordinate+coords[i])for i in range(types_before.__len__())])
                #    E_before = sum([self.Vs[types_before[i]][self.outer_class.I(global_coordinate+coords[i])] for i in range(types_before.__len__())])
                #    E_after = sum([self.Vs[types_after[i]][self.outer_class.I(global_coordinate+coords[i])] for i in range(types_before.__len__())])
                #    return rate_constant*np.exp((-E_after+E_before)/2.)
